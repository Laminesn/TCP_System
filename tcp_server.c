...
int main(void)
{
   ...
   // struct sockaddr_in server_addr, client_addr;
    ...
    // previous external temperatures to break loop
    float prevTemp1 = 0.0;
    float prevTemp2 = 0.0;
    float prevTemp3 = 0.0;
    float prevTemp4 = 0.0;
   ...
   int stable = false;
	...
	//check if all temperature are similar then break loop
	if ( (prevTemp1 == temperature[0]) && (prevTemp2 == temperature[1]) && (prevTemp3 == temperature[2]) && 
			(prevTemp4 == temperature[3]) ) {
		break;
	}

	// change prevTemperatures
	prevTemp1 = temperature[0];
	prevTemp2 = temperature[1];
	prevTemp3 = temperature[2];
	prevTemp4 = temperature[3];

        // Modify Temperature
	float totalTemp = temperature[0] + temperature[1] + temperature[2] + temperature[3]; 
        float updatedTemp = ( (2 * centralTemp) + totalTemp ) / 6;

	// Update central temp for looping
